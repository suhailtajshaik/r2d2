...
get_openclaw_config() {
    if [[ -f /home/r2d2/.openclaw/openclaw.json ]]; then
        local port=$(jq -r '.gateway.port // 18789' /home/r2d2/.openclaw/openclaw.json 2>/dev/null)
        local model=$(jq -r '.agents.defaults.model.primary // "unknown"' /home/r2d2/.openclaw/openclaw.json 2>/dev/null)
        local fallbacks=$(jq -r '.agents.defaults.model.fallbacks // [] | map(split("/")[1] // .) | join(" → ")' /home/r2d2/.openclaw/openclaw.json 2>/dev/null)
...
output_standard() {
    ...
    if [[ -n "$pid" ]]; then
        ...
        local sessions=$(ls /home/r2d2/.openclaw/agents/main/sessions/*.json 2>/dev/null | wc -l || echo "0")

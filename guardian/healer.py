...
def attempt_heal(check_name, message):
    ...
    heal_map = {
        ...
        "Maxwell Headlines Today": heal_headlines_today_generator,
    }
...
Then document the result in /home/r2d2/guardian/logs/guardian-notifications.log. Do not use OpenClaw; it has been removed from this VPS.
"""
...
def heal_memory_write(message):
    ...
    import subprocess
    subprocess.run(["mkdir", "-p", "/home/r2d2/brain/workspace/memory"])
    subprocess.run(["chmod", "-R", "755", "/home/r2d2/brain/workspace/memory"])
    return True

def heal_headlines_today_generator(message):
    """Ask Maxwell to regenerate The Headlines Today PDF/audio/data."""
    result = subprocess.run(["python3", "/home/r2d2/projects/news-engine/generate-dev-edition.py"], cwd="/home/r2d2/projects/news-engine", capture_output=True, text=True, timeout=180)
    if result.returncode == 0:
        log.info("Maxwell regenerated The Headlines Today successfully")
        return True
    log.error(f"Maxwell regeneration failed: {result.stderr.strip() or result.stdout.strip()}")
    call_3po("Maxwell failed to regenerate The Headlines Today. Run: python3 /home/r2d2/projects/news-engine/generate-dev-edition.py — and fix any errors.")
    return False
...
7. Record a brief local notification in /home/r2d2/guardian/logs/guardian-notifications.log. Do not use OpenClaw; it has been removed from this VPS.

...
<body class="bg-gray-50 text-gray-800 dark:bg-gray-900 dark:text-gray-100 transition-colors duration-300">
    ...
    {f"""<section class="bg-white dark:bg-gray-800 border-y border-gray-100 dark:border-gray-700">
        <div class="max-w-4xl mx-auto px-6 py-16">
            <h2 class="text-2xl font-bold mb-8">Education & Certifications</h2>
            <div class="grid md:grid-cols-2 gap-6">
                {f'<div><h3 class="font-semibold text-lg mb-3">Education</h3>{"".join(f"<p class=\\"text-gray-600 dark:text-gray-300 mb-2\\">{escape(e)}</p>" for e in education)}</div>' if education else ''}
                {f'<div><h3 class="font-semibold text-lg mb-3">Certifications</h3>{"".join(f"<p class=\\"text-gray-600 dark:text-gray-300 mb-2\\">{escape(c)}</p>" for c in certifications)}</div>' if certifications else ''}
            </div>
        </div>
    </section>""" if education or certifications else ''}
